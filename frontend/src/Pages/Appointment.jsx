...
function Appointment() {
  ...
  return Doctorinformation && (
    <div >
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {
            docSlot.length && docSlot.map((item,idx)=>(
              <div onClick={()=>setslotidx(idx)} className={`text-center py-5 min-w-16 rounded-full cursor-pointer ${slotidx===idx ? 'bg-primary text-white':'border border-gray-200'}`} key={idx}>
      ...
      <RelatedDoctors docId={docId} speciality={Doctorinformation.speciality}/>
